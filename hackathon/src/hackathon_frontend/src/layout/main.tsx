...
const Template = ({ children }: TemplateProps) => {
    var [actived, setActived] = useState('-right-[calc(75%+32px)] md:-right-[calc(60%+32px)]')
    const [isActive, setIsActive] = useState(true)

    const handleClick = () => {
        setIsActive(!isActive)

        isActive? setActived(actived = 'right-8') : setActived(actived = '-right-[calc(75%+24px)] md:-right-[calc(60%+24px)]')
    }
        ...
        <div className="w-full h-full flex flex-col justify-center items-center">
            <nav className="fixed top-0 flex justify-center w-screen bg-white/30 backdrop-blur-2xl shadow-lg z-50">
                <div className="px-8 lg:px-20 h-20 max-w-[1728px] w-full flex items-center justify-between text-[#145374] font-bold">
                    ...
                    <a href="" className="group relative hidden lg:flex px-10 py-1.5 items-center justify-center bg-[#FEF7EF] overflow-hidden border-2 border-white rounded-xl font-bold text-xl shadow-lg transition-all duration-500 hover:-translate-y-0.5">
                    ...
                    <button onClick={handleClick} className="rounded-full block lg:hidden p-2 hover:bg-white/50">
                        <Bars3Icon className='size-10 text-[#145374]'/>
                    </button>
                </div>
            </nav>
            <nav className={"fixed flex lg:hidden flex-col items-center rounded-4xl py-10 top-28 w-[75%] md:w-[60%] bg-gradient-to-br from-white/30 to-[#F4D06F]/30 backdrop-blur-2xl z-[60] border-4 border-white/50 shadow-lg text-white space-y-8 transition-all ease-in-out delay-100 duration-1000 " + actived}>
                <div className="w-full flex justify-between items-center px-10">
                    <h1 className="font-bold text-[#F81]/85 text-xl md:text-2xl">Explore <br /> Finechain</h1>
                    <a href="" className="group relative w-fit px-6 py-3 flex items-center justify-center bg-[#FEF7EF] overflow-hidden border-2 border-white rounded-xl font-bold text-xl shadow-lg transition-all duration-500 hover:-translate-y-0.5">
                ...
                <div className="w-full flex flex-col text-xl md:text-2xl font-semibold pb-5">
                    <a href="" className="w-full h-20 px-10 flex items-center hover:bg-white/30">Home</a>
                    <a href="" className="w-full h-20 px-10 flex items-center hover:bg-white/30">Fines</a>
                    <a href="" className="w-full h-20 px-10 flex items-center hover:bg-white/30">Payment</a>
                    <a href="" className="w-full h-20 px-10 flex items-center hover:bg-white/30">History</a>
                </div>
            </nav>
            ...
            <footer className="relative w-full h-fit flex justify-center z-30">
                <div className="w-full h-full max-w-[1728px] flex flex-col items-center bg-linear-to-r from-gray-300/40 to-white/40 backdrop-blur-xl rounded-t-[48px] border-t-8 border-white/50 shadow-lg pt-20 space-y-8">
                    <div className="flex flex-col lg:flex-row gap-12 justify-between w-full h-full px-8 lg:px-20">
                        <img src="/footer_ornament.svg" className="w-44 xl:w-52 object-top opacity-100" />
                        <div className="flex flex-col md:flex-row justify-center lg:justify-end gap-12 md:gap-20 w-full text-[#145374] text-xl font-medium">
                            <div className="space-y-4 md:space-y-6 md:max-w-56">
                                ...
                                <p>Each transaction are encrypted, transparent, and tamper-proof</p>
                            </div>
